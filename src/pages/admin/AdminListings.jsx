...
const AdminListings = () => {
  ...
  const fetchListings = useCallback(() => adminService.getListings({ showArchived }), [showArchived]);
  const {
    data: cachedListings,
    isLoading: loading,
    isRefreshing,
  ...
  } = useCachedPageData(`admin-listings-${showArchived}`, fetchListings, {
  ...
  const handleArchive = async (listingId) => {
    if (!window.confirm('Are you sure you want to archive this listing? It will no longer be visible to users.')) return;
    try {
      await adminService.archiveListing(listingId);
      invalidatePageCaches([
        'admin-dashboard',
        `admin-listings-${showArchived}`,
        'host-listings',
        'host-dashboard',
        'listings:active-approved',
        'charger-detail',
  ...
  const handleUnarchive = async (listingId) => {
    try {
      await adminService.unarchiveListing(listingId);
      invalidatePageCaches([
        'admin-dashboard',
        `admin-listings-${showArchived}`,
        'host-listings',
  ...
  const getStatus = (listing) => {
    if (listing.isArchived || listing.archivedAt) return { label: 'Archived', color: '#6b7280' };
                ...
                {filtered.map(listing => {
                  ...
                  return (
                    <tr key={listing.id} style={{ borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                      ...
                      <td style={{ padding: '0.75rem' }}>
                        <div style={{ display: 'flex', gap: '0.5rem', flexWrap: 'wrap' }}>
                          {!listing.isApproved && listing.setupFeePaid && !listing.isArchived && (
                            <>
                          ...
                          {!listing.isArchived ? (
                            <button onClick={() => handleArchive(listing.id)} style={{ padding: '0.3rem 0.6rem', borderRadius: '4px', border: '1px solid rgba(156,163,175,0.3)', background: 'rgba(156,163,175,0.1)', color: '#9ca3af', cursor: 'pointer', fontSize: '0.75rem', fontFamily: 'var(--font-body)' }}>Archive</button>
                          ) : (
                            <button onClick={() => handleUnarchive(listing.id)} style={{ padding: '0.3rem 0.6rem', borderRadius: '4px', border: '1px solid rgba(59,130,246,0.3)', background: 'rgba(59,130,246,0.1)', color: '#60a5fa', cursor: 'pointer', fontSize: '0.75rem', fontFamily: 'var(--font-body)' }}>Unarchive</button>
                          )}
                        </div>
                      </td>
                    </tr>
