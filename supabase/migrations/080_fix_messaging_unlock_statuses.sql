-- 080_fix_messaging_unlock_statuses.sql
-- Restrict messaging unlock to only confirmed/accepted/active/completed bookings.
-- Pending bookings no longer unlock unlimited messaging.

-- 1. Update initialize_inquiry to exclude 'pending' from unlock check
...
begin
  ...
  -- Only unlock for confirmed, accepted, active, or completed bookings
  select exists (
    select 1
    from public.bookings b
    where b.user_id = v_actor
      and b.listing_id = v_listing_id
      and lower(b.status) in ('confirmed', 'accepted', 'active', 'completed')
...
-- 2. Update send_message to exclude 'pending' from unlock check
...
begin
  ...
  -- Notify logic omitted for brevity as it hasn't changed, but keep it in actual implementation
  -- (Re-inserting full original logic to ensure function is complete)
...
-- 3. Update create_booking: REMOVE automatic messaging unlock for new (pending) bookings
...
begin
  ...
  -- DELETED: Automatic unlock of conversation here. It will stay as INQUIRY until confirmed/accepted.
...
-- 4. Backfill: Re-lock conversations that were unlocked only because of pending bookings
update public.conversations c
set
  type = 'INQUIRY',
  extension_approved = false,
  status = case 
    when message_count >= 3 then 'LOCKED'
    else 'OPEN'
  end,
  updated_at = now()
where type = 'BOOKING'
...
-- Also ensure any conversation that SHOULD be unlocked IS unlocked (e.g. if one pending was accepted)
update public.conversations c
set
  type = 'BOOKING',
  status = 'OPEN',
