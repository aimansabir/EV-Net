-- 082_booking_revenue_model.sql
-- Separate Platform Revenue vs Host Payout and add payout tracking

-- 1. Add payout_status to bookings
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS payout_status text DEFAULT 'pending'
    CONSTRAINT valid_payout_status CHECK (payout_status IN ('pending', 'paid_to_host'));

-- 2. Backfill existing completed bookings (safety)
UPDATE public.bookings 
SET payout_status = 'pending' 
WHERE status = 'COMPLETED' AND payout_status IS NULL;

-- 3. Verify Payment RPC (Host action)
...
BEGIN
  ...
  INSERT INTO public.booking_status_history (
    ...
    p_booking_id, v_booking.status, v_booking.status, v_user_id, 'Host verified payment proof'
  );
  ...
  INSERT INTO public.notifications (
    ...
    'PAYMENT', 
    'Payment Verified',
    format('Your payment for %s has been verified by the host.', v_listing_title),
    jsonb_build_object(
...
-- 4. Complete Booking RPC (Host action)
...
BEGIN
  ...
  -- Validation: Must be confirmed and paid (or pay after charging)
  IF v_booking.status NOT IN ('CONFIRMED', 'PENDING') THEN
    RAISE EXCEPTION 'Booking must be CONFIRMED or PENDING to be marked as completed.';
  END IF;

  IF v_booking.payment_status != 'paid' AND v_booking.payment_method != 'PAY_AFTER_CHARGING' THEN
  ...
  SET status = 'COMPLETED',
      payment_status = CASE 
                         WHEN payment_method = 'PAY_AFTER_CHARGING' THEN 'paid' 
                         ELSE payment_status 
                       END,
      updated_at = now()
  WHERE id = p_booking_id
